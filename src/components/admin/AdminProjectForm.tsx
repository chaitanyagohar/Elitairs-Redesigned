...
// ✅ Defined Builder Options Constant
const BUILDER_OPTIONS = ["Sobha", "Emaar", "Godrej"];
...
export default function AdminProjectForm({ initialData }: { initialData?: any }) {
  ...
  // 1. Determine Builder Logic
  const initialBuilder = initialData?.builder || "Sobha";
  const isCustomBuilder = !BUILDER_OPTIONS.includes(initialBuilder) && initialBuilder !== "Other";
  ...
  const [formData, setFormData] = useState({
    ...
    builder: isCustomBuilder ? "Other" : initialBuilder,
    otherBuilder: isCustomBuilder ? initialBuilder : "",
    ...
    <>
      ...
      {previewImage && (
        ...
        >
          <button 
            onClick={() => setPreviewImage(null)} 
            className="absolute top-5 right-5 text-white bg-white/20 hover:bg-white/40 p-2 rounded-full w-10 h-10 flex items-center justify-center transition-colors"
          >
            ✕
          </button>
          <img 
      ...
      <form onSubmit={handleSubmit} className="max-w-6xl mx-auto bg-white p-4 sm:p-10 rounded-xl shadow-sm border border-gray-200 text-black">
        ...
        <div className="mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
               ...
               {formData.builder === "Other" && (
                  <input 
                      name="otherBuilder" 
                      value={formData.otherBuilder} 
        ...
        <div className="mb-10">
          ...
          {/* ✅ FLOOR PLAN SECTION WITH LIGHTBOX */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                {formData.floorPlans.map((plan: any, i: number) => (
                  <div key={i} className="flex gap-4 items-start bg-white p-3 rounded border">
                     ...
                     {/* ✅ Clickable Lightbox Trigger */}
                     <div 
                        onClick={() => setPreviewImage(plan.url)}
                        className="block w-24 h-24 shrink-0 bg-gray-100 rounded border overflow-hidden hover:opacity-90 relative group cursor-pointer"
                     >
                         <img src={plan.url} className="w-full h-full object-cover" alt="floor plan thumb" />
                         <div className="absolute inset-0 flex items-center justify-center bg-black/10 group-hover:bg-black/20">
                            <span className="bg-black/60 text-white text-[10px] px-1 rounded opacity-0 group-hover:opacity-100">View</span>
                         </div>
                     </div>
