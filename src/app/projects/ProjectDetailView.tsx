...
const TabIcon = ({ type }: { type: string }) => {
  const color = "#B08D55";
  switch (type) {
    case "connectivity": return <svg width="14" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2"><path d="M12 21s-6-5.33-6-10a6 6 0 1 1 12 0c0 4.67-6 10-6 10z" /><circle cx="12" cy="11" r="2.5" /></svg>;
    case "schools": return <svg width="14" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2"><path d="M22 10L12 4 2 10l10 6 10-6z" /><path d="M6 12v5c0 1.1 2.7 2 6 2s6-.9 6-2v-5" /></svg>;
    case "hospitals": return <svg width="14" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2"><path d="M12 2v20M2 12h20" /></svg>;
    case "malls": return <svg width="14" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2"><path d="M6 2l1 4h10l1-4" /><path d="M3 6h18l-1 14H4L3 6z" /></svg>;
    default: return null;
  }
};
...
export default function ProjectDetailView({ project, similarProjects }: any) {
  ...
  const visualAmenities = (project?.projectAmenities ?? []).filter((am: any) => am && (am.title || am.name) && (am.image || am.icon));
                ...
                <div className="flex flex-col lg:grid lg:grid-cols-3 bg-white rounded-xl shadow-xl overflow-hidden border border-gray-100 h-auto lg:h-[500px]">
                    ...
                    <div className="lg:col-span-1 flex flex-col h-[400px] lg:h-full">
                        <div className="flex border-b overflow-x-auto scrollbar-hide">
                            {[{ label: "Connectivity", key: "connectivity" }, { label: "Schools", key: "schools" }, { label: "Hospitals", key: "hospitals" }, { label: "Malls", key: "malls" }].map((tab) => (
                                <button key={tab.key} onClick={() => setActiveLocalityTab(tab.key)} className={`flex-1 min-w-[90px] py-4 text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-2 whitespace-nowrap ${activeLocalityTab === tab.key ? "bg-[#FFC40C] text-black" : "bg-white text-gray-500 hover:text-black"}`}>
                                    <TabIcon type={tab.key} /> {tab.label}
                                </button>
                            ))}
                        </div>
                        <div className="p-6 overflow-y-auto flex-1 bg-white">
                            <ul className="space-y-4">
                                {getLocalityData().length ? getLocalityData().map((item: string, i: number) => (
                                    <li key={i} className="flex justify-between items-center text-sm border-b border-gray-100 pb-2 last:border-0">
                                        <span className="text-gray-700 font-medium">{item}</span><span className="text-[#FFC40C] text-xs">●</span>
                                    </li>
                                )) : <li className="text-gray-400 italic text-center mt-10">No data available for this category.</li>}
                            </ul>
                        </div>
                    </div>
                </div>
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-1 relative">
                  <div className="rounded-lg overflow-hidden bg-gray-800 relative w-full h-[220px] sm:h-[300px] md:h-[420px] lg:h-[520px]" onMouseEnter={handleInteractionStart} onMouseLeave={handleInteractionEnd}>
                    ...
                    />
                    <div className="absolute left-0 right-0 bottom-6 flex items-center justify-center gap-2 z-10">
                    ...
                    <button onClick={() => openLightbox(galleryIndex)} className="absolute right-4 bottom-4 text-white bg-black/40 p-2 rounded-md z-10">⤢</button>
                  </div>
